...
def _coerce(v):
    """Keep formulas as text; turn numeric-looking text (e.g. '1,250.50') into numbers."""
...
@tool("create_excel",
      "Create an Excel (.xlsx) workbook. Each sheet has column headers and rows. Cells may contain "
      "Excel formulas like '=SUM(B2:B10)'. Optionally add a bar/line/pie chart per sheet.",
    ...
    for spec in sheets:
        ...
        if chart_spec and ws.max_row > 1:
            ...
            for vc in value_cols[:1] if kind == "pie" else value_cols:
                chart.add_data(Reference(ws, min_col=int(vc), min_row=1, max_row=ws.max_row), titles_from_data=True)
            chart.set_categories(Reference(ws, min_col=label_col, min_row=2, max_row=ws.max_row))
            chart.height, chart.width = 8, 16
            ws.add_chart(chart, f"{get_column_letter(len(cols) + 2)}2")
        summary.append(f"{ws.title}: {ws.max_row - 1} rows x {len(cols)} columns" + (f", {kind} chart" if chart_spec else ""))
